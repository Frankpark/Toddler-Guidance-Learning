...
if LOAD_VISION:
    from agents import LoadableEncoder as Encoder
else:
...
class Model():
    def __init__(self, envs): 
        ...
        if LOAD_VISION:
            with open('../unsup_embedding_vision/model_vision.pkl', 'rb') as F:
                vision_data = pkl.load(F)
            self.load_vision(vision_data)
        self.updateNetwork()
    ...
    def load_vision(self, vision_data):
        self.encoderC1.load_vision(self.sess, vision_data)
        self.encoderC2.load_vision(self.sess, vision_data)
        self.encoderA.load_vision(self.sess, vision_data)
        self.encoderC10.load_vision(self.sess, vision_data)
        self.encoderC20.load_vision(self.sess, vision_data)
        self.encoderA0.load_vision(self.sess, vision_data)
